...
const MessageContainer = () => {
  ...
  const checkIfImage = (filePath) => {
    const imageRegex = /\.(jpg|jpeg|png|gif|bmp|tif|tiff|webp|svg|ico|heic|heif)$/i;
...
const downloadFile = async (url) => {
  try {
    ...
    const response = await apiClient.get(`${HOST}/${url}`, {
      ...
      onDownloadProgress: (progressEvent) => {
        if (progressEvent.total) {
          ...
          console.log("Download Progress:", percentCompleted); // Debugging line
    ...
    setFileDownloadProgress(0); // Ensure it reaches 100% after completion
  } catch (error) {
